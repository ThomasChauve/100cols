...
if st.button('Trouver et supprimer les doubles'):
    ...
    st.session_state['data_list'][st.session_state['id_u']].cols=st.session_state['data_list'][st.session_state['id_u']].cols.drop_duplicates(subset=['Code'], keep='first')
    st.success('Double supprimé :'+str(bf-len(st.session_state['data_list'][st.session_state['id_u']].cols)), icon="✅")